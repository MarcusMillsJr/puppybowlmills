...
export const renderAllPlayers = (playerList) => {
    ...
    for (let i = 0; i < playerList.length; i++) {
      ...
      <div class="single-player-card">
        ...
        <button class="delete-button" data-id=${pup.id}>Remove Button</button>
      </div>
    `;
    ...
    for (let i = 0; i < detailButtons.length; i++) {
        ...
        button.addEventListener("click", async () => {
            const fetchPlayer = await fetchSinglePlayer(button.dataset.id);
            renderSinglePlayer(fetchPlayer);
        });
    }
...
export const renderSinglePlayer = (playerObj) => {
  ...
  `;
    ...
    const returnToAll = document.getElementById("see-all");
    returnToAll.addEventListener("click", async () => {
